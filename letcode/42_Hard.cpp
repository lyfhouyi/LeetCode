...
//双指针遍历数组。计算当前指针位置可以接到的最大水量。当左壁最大值小于右壁最大值时，左侧指针可以计算，因为其水量限制于其左壁最大值，且其左壁最大值已不可能更大；同理，当右壁最大值小于左壁最大值时，右侧指针可以计算，因为其水量限制于其右壁最大值，且其右壁最大值已不可能更大；两个指针中，必有且仅有一个指针可以计算，则计算该指针，并移动该指针，直到该指针不能计算时，计算另一个指针。
//int trap(vector<int>& height)
//{
//	int n = height.size();
//	int sumWater = 0;
...
//递减栈。单指针遍历数组，计算以当前指针为全局右壁时的最大盛水量，则当该指针移动至数组尾部时，所得即为数组的盛水量。当当前指针（作为右壁）的高度小于栈顶时，将当前指针处的高度压栈，否则逐个计算栈顶位置的盛水量，直到栈顶高度高于当前指针处的高度。
int trap(vector<int>& height)
{
	int n = height.size();
